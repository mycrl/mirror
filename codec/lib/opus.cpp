...
AudioDecoder* codec_create_audio_decoder(AudioDecoderSettings* settings)
{
    AudioDecoder* codec = new AudioDecoder{};
    codec->output_frame = new AudioFrame{};

    const AVCodec* decoder = avcodec_find_decoder_by_name(settings->codec);
    if (decoder == nullptr)
    {
        return nullptr;
    }
    ...
    if (avcodec_open2(codec->context, decoder, nullptr) != 0)
    {
        codec_release_audio_decoder(codec);
        return nullptr;
    }
    ...
    codec->parser = av_parser_init(decoder->id);
    if (!codec->parser)
    {
        codec_release_audio_decoder(codec);
        return nullptr;
...
AudioEncoder* codec_create_audio_encoder(AudioEncoderSettings* settings)
{
    ...
    const AVCodec* encoder = avcodec_find_encoder_by_name(settings->codec);
    if (encoder == nullptr)
    {
        return nullptr;
    }

    codec->context = avcodec_alloc_context3(encoder);
    if (!codec->context)
    {
    ...
    if (avcodec_open2(codec->context, encoder, nullptr) != 0)
    {
        codec_release_audio_encoder(codec);
        return nullptr;
    }
