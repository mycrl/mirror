...
impl VideoRender {
    pub fn new(handle: &WindowHandle) -> Result<Self> {
        if unsafe { SDL_Init(SDL_INIT_VIDEO) } != 0 {
            return error();
        }

        let (window, size) = unsafe {
            let (w, s) = match handle {
                WindowHandle::Win32(hwnd, width, height) => (
                    *hwnd,
                    Size {
                        width: *width,
                        height: *height,
